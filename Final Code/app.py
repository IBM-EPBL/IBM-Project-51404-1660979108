...
conn=ibm_db.connect("DATABASE=bludb;HOSTNAME=6667d8e9-9d4d-4ccb-ba32-21da3bb5aafc.c1ogj3sd0tgtu0lqde00.databases.appdomain.cloud;PORT=30376;SECURITY=SSL;SSLServerCertificate=certificatessl.crt;UID=qnc73201;PWD=71Pug2z1w7EuVyLD;",'','')
#Index
@app.route('/')
def index():
...
def add_product_movements():
    ...
    if request.method == 'POST' and form.validate():
        ...
        if from_location==to_location:
            ...
            alert=current_num['PRODUCT_NUM']-qty

            if(alert<=0):
                alert("Please update the quantity of the product {}, Atleast {} number of pieces must be added to finish the pending Product Movements!".format(product_id,-alert))
